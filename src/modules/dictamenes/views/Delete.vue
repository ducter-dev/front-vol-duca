...
const { deleteDictamen } = useDictamen()
const { addToast } = useToast()

let loader = ref(false)
let isOpen = ref(false)

const props = defineProps(['model', 'id'])
...
const submit = async () => {
  try {
        ...
        const { data, status, message } = await deleteDictamen(props.id)
        if (status == 200) {
          addToast({
            message: {
              title: "Éxito!",
              message: `Se eliminó el dictamen con folio ${data.folioDictamen} del listado.`,
              type: "success"
            },
          })
          isOpen.value = false
...
<template>
  <Modal size="lg" position="center" @submit="submit" @openModal="toggleModal()" @closeModal="closeModal()"
    ...
    <template #title>Eliminar Dictamen</template>
    <template #content>
      <p class="text-sm text-gray-500">
        ¿Está seguro que desea eliminar el dictamen: <strong>{{ model.folioDictamen }}</strong>?
      </p>
    </template>
    <template #actions>
      <Button variant="danger" :disabled="loader">
        <template v-if="loader">
          <svg class="inline w-5 h-5 text-white animate-spin" viewBox="0 0 100 101" fill="none"
