...
const { fetchBitacoras, selectBitacora } = useBitacora()
...
const fetchDataBitacora = async () => {
  try {
    ...
    const res = await fetchBitacoras(params)
...
const goToInsert = () => {
  router.push({ name: 'bitacora.create'})
}

const goToEdit = (item) => {
  setClienteSelected(item)
  router.push({ name: 'bitacora.edit'})
}

const setDescripcion = (item) => {
  return `${item.descripcion1} ${item.descripcion2} ${item.descripcion3}`
}
...
<template>
        ...
        <div class="p-1 bg-white border shadow border-slate-200 dark:bg-slate-800 dark:border-slate-700">
          <div class="border border-solid border-slate-300">
            <div class="flex items-center justify-between">
              <legend class="p-2 text-base font-medium text-slate-900 dark:text-white">Lista de bitacora
              </legend>
              <div>
            ...
            <LTable :loader="loading">
              ...
              <template #body>
                <tr v-for="item in bitacoras" v-if="bitacoras.length > 0" :key="item.id">
                  <LBodyTh :value="item.id" center />
                  <td>
                    {{ setDescripcion(item) }}
                  </td>
                  <LBodyTd :value="item.evento.descripcion" center />
                  <LBodyTd :value="item.usuario.usuario" center />
  ...
  <Paginate 
    ...
    :offset="7"
    @changePaginate="fetchDataBitacora()" />
</template>