...
import DeleteCliente from './Delete.vue'

/* Declaración de atributos asignables */
const { bus } = useEventsBus()
const { fetchClientes, setClienteSelected } = useCliente()
...
/**
 *  Función que consulta `fetchDataClientes`  para obtener datos desde la API la información.
...
const goToInsert = () => {
  router.push({ name: 'clientes.create'})
}

const goToEdit = (item) => {
  setClienteSelected(item)
  router.push({ name: 'clientes.edit'})
}
 ...
 *  en caso contrario se invoca a la funcion @function fetchDataClientes 
        ...
        <div class="p-1 bg-white border shadow border-slate-200 dark:bg-slate-800 dark:border-slate-700">
          <div class="border border-solid border-slate-300">
            <div class="flex items-center justify-between">
              <legend class="p-2 text-base font-medium text-slate-900 dark:text-white">Lista de clientes
              </legend>
              <div>
            ...
            <LTable :loader="loading">
              ...
              <template #body>
                <tr v-for="item in clientes" v-if="clientes.length > 0" :key="item.id">
                  <LBodyTh :value="item.id" center />
                  <LBodyTh :value="item.nombreCliente" center />
                  <LBodyTh :value="item.rfcCliente" center />
                  <LBodyTd center>
