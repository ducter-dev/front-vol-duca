...
import FormCliente from '../components/FormClientes.vue'

const router = useRouter()
const { addToast } = useToast()
const { emit, bus } = useEventsBus()
const loader = ref(false)
const { insertCliente } = useCliente()

/* Declaración de métodos */
async function onSubmit(form) {
  loader.value = true
  const { data, status, message } = await insertCliente(form)
  if (status == 200) {
    addToast({
      message: {
        title: "Éxito!",
        message: `Se agregó el cliente ${data.nombreCliente} a nuestros registros.`,
        type: "success"
      },

    })
    loader.value = false
    router.push({ name: 'clientes.home' })
  } else {
    loader.value = false
    addToast({
      message: {
