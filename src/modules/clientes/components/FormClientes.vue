...
const { getClientesSelected } = useCliente()
const clienteSelected = computed(() => getClientesSelected())
...
const clienteForm = reactive(initialFormData())

if (Object.keys(clienteSelected.value).length > 0) {
  clienteForm.id = clienteSelected.value.id
  clienteForm.rfcCliente = clienteSelected.value.rfcCliente
  clienteForm.nombreCliente = clienteSelected.value.nombreCliente
}

const resetform = () => {
  Object.assign(clienteForm, initialFormData())
}
const handleSubmit = () => {
  const obj = {
    id: clienteForm.id,
    rfcCliente: clienteForm.rfcCliente,
    nombreCliente: clienteForm.nombreCliente,
  }
  console.log("🚀 ~ file: FormClientes.vue:51 ~ handleSubmit ~ obj:", obj)
  emit('submitForm', obj)
  resetform()
}
  ...
  <form class="w-8/12 xl:w-6/12" @submit.prevent="handleSubmit">
    <div class="relative z-0 w-full mb-6 group">
      <input type="text" name="nombre"
        ...
        placeholder=" " required v-model="clienteForm.nombreCliente" />
      <label
    ...
    <div class="relative z-0 w-full mb-6 group">
      <input type="text" name="descripcion"
        class="block py-2.5 px-0 w-full text-base text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
        placeholder=" " required v-model="clienteForm.rfcCliente" />
      <label
        class="peer-focus:font-medium absolute text-base text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">RFC
        del Cliente</label>
    </div>
