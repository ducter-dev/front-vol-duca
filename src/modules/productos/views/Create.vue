...
const { insertProducto, syncCompuestos } = useProducto()

/* Declaración de métodos */
async function onSubmit(form) {
  loader.value = true
  const { data, status, message } = await insertProducto(form)
  if (status == 200) {
    const obj = {
      producto: data.id,
      data: form.compuestos
    }
    const respSync = await syncCompuestos(obj)
    const statusSync = respSync.status
    const messageSync = respSync.message

    if (statusSync == 200) {
      addToast({
        message: {
          title: "Éxito!",
          message: `Se agregó el producto ${data.descripcion} a nuestros registros.`,
          type: "success"
        },
      })
      loader.value = false
      router.push({ name: 'productos.home' })
    } else {
      addToast({
        message: {
          title: "¡Error!",
          message: message,
          type: "error",
          component: "create compuestos - onSubmit()"
        },
      })
      loader.value = false
      router.push({ name: 'productos.home' })
    ...
    addToast({
      message: {
        ...
        message: message,
        type: "error",
        component: "create producto - onSubmit()"
      },
    })
  }
}
