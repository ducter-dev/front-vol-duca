...
const submit = async () => {
  try {
        loader.value = true
        const { data, status, message } = await deleteProducto(props.id)
        if (status == 200) {
          addToast({
            message: {
              title: "Éxito!",
              message: `Se eliminó el producto ${data.descripcion} de nuestros registros.`,
              type: "success"
            },
          })
          isOpen.value = false
  ...
  <Modal size="lg" position="center" @submit="submit" @openModal="toggleModal()" @closeModal="closeModal()"
    ...
    <template #content>
      <p class="text-sm text-gray-500">
        ¿Está seguro que desea eliminar el producto: <strong>{{ model.descripcion }}</strong>?
      </p>
    </template>
    <template #actions>
