...
const { deletePermiso } = usePermiso()
...
const submit = async () => {
  try {
        loader.value = true
        const { data, status, message } = await deletePermiso(props.id)
        if (status == 200) {
          addToast({
            message: {
              title: "Éxito!",
              message: `Se eliminó el permiso ${data.nombre} del listado.`,
              type: "success"
            },
          })
          isOpen.value = false
  ...
  <Modal size="lg" position="center" @submit="submit" @openModal="toggleModal()" @closeModal="closeModal()"
    ...
    <template #content>
      <p class="text-sm text-gray-500">
        ¿Está seguro que desea eliminar el permiso: <strong>{{ model.nombre }}</strong>?
      </p>
    </template>
    <template #actions>
