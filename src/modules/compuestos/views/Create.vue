...
import FormCompuestos from '../components/FormCompuestos.vue'

const router = useRouter()
const { addToast } = useToast()
const { emit, bus } = useEventsBus()
const loader = ref(false)
const { insertCompuesto } = useCompuesto()

/* Declaración de métodos */
async function onSubmit(form) {
  loader.value = true
  const { data, status, message } = await insertCompuesto(form)
  if (status == 200) {
    addToast({
      message: {
        title: "Éxito!",
        message: `Se agregó el compuesto ${data.descripcion} a nuestros registros.`,
        type: "success"
      },

    })
    loader.value = false
    router.push({ name: 'compuestos.home' })
  } else {
    loader.value = false
    addToast({
      message: {
