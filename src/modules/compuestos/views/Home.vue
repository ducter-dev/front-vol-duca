...
import useCompuesto from "../composables/compuestos"
import Delete from './Delete.vue'

/* Declaración de atributos asignables */
const { bus } = useEventsBus()
const { fetchCompuestos, setCompuestoSelected } = useCompuesto()
...
const setDataFromResult = (data) => {
  ...
  compuestos.value = data
  loading.value = false
}

/**
 *  Función que consulta `fetchDataCompuestos`  para obtener datos desde la API la información.
...
const fetchDataCompuestos = async () => {
  try {
    loading.value = true
    const params = { page: pagination.current_page}
    const res = await fetchCompuestos(params)
...
const goToInsert = () => {
  router.push({ name: 'compuestos.create'})
}

const goToEdit = (item) => {
  setCompuestoSelected(item)
  router.push({ name: 'compuestos.edit'})
}
 ...
 *  en caso contrario se invoca a la funcion @function fetchDataCompuestos 
...
<template>
        ...
        <div class="p-1 bg-white border shadow border-slate-200 dark:bg-slate-800 dark:border-slate-700">
          <div class="border border-solid border-slate-300">
            <div class="flex items-center justify-between">
              <legend class="p-2 text-base font-medium text-slate-900 dark:text-white">Lista de compuestos
              </legend>
              <div>
                ...
                <button class="p-2" @click="fetchDataCompuestos()">
            ...
            <LTable :loader="loading">
              ...
              <template #body>
                <tr v-for="item in compuestos" v-if="compuestos.length > 0" :key="item.id">
                  ...
                  <LBodyTd center>
                    <div class="inline-flex shadow-sm" role="group">
                      ...
                      <Delete :model="item" :id="item.id" @successSubmit="fetchDataCompuestos()" />
                    </div>
                  </LBodyTd>
                </tr>
                <tr v-else>
  ...
  <Paginate 
    ...
    :offset="7"
    @changePaginate="fetchDataCompuestos()" />
</template>