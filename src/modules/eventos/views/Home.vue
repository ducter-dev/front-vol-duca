...
import DeleteEvento from './Delete.vue'

/* Declaración de atributos asignables */
const { bus } = useEventsBus()
const { fetchEventos, setEventoSelected } = useEvento()
...
/**
 *  Función que consulta `fetchDataEventos`  para obtener datos desde la API la información.
...
const goToInsert = () => {
  router.push({ name: 'eventos.create'})
}
 ...
 *  en caso contrario se invoca a la funcion @function fetchDataEventos 
...
<template>
        ...
        <div class="p-1 bg-white border shadow border-slate-200 dark:bg-slate-800 dark:border-slate-700">
          <div class="border border-solid border-slate-300">
            <div class="flex items-center justify-between">
              <legend class="p-2 text-base font-medium text-slate-900 dark:text-white">Lista de eventos
              </legend>
              <div>
                ...
                <button class="p-2" @click="fetchDataEventos()">
            ...
            <LTable :loader="loading">
              <template #head>
                <tr>
                  ...
                  <LHeaderTh value="Descripcion" center />
                  <LHeaderTh value="Acciones" center />
                </tr>
              </template>
              <template #body>
                <tr v-for="item in eventos" v-if="eventos.length > 0" :key="item.id">
                  ...
                  <LBodyTd center>
                    <div class="inline-flex shadow-sm" role="group">
                      ...
                      <DeleteEvento :model="item" :id="item.id" @successSubmit="fetchDataEventos()" />
                    </div>
                  </LBodyTd>
                </tr>
                <tr v-else>
  ...
  <Paginate 
    ...
    :offset="7"
    @changePaginate="fetchDataEventos()" />
</template>