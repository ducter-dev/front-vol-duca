...
const { deleteAlmacen } = useAlmacenes()
...
const submit = async () => {
  try {
        loader.value = true
        const { data, status, message } = await deleteAlmacen(props.id)
        if (status == 202) {
          addToast({
            message: {
              title: "Éxito!",
...
<template>
  <Modal size="lg" position="center" @submit="submit" @openModal="toggleModal()" @closeModal="closeModal()"
    ...
    <template #title>Eliminar Almacén</template>
    <template #content>
      <p class="text-sm text-gray-500">
        ¿Está seguro que desea eliminar el almacén: <strong>{{ model.id }}</strong>?
      </p>
    </template>
    <template #actions>
