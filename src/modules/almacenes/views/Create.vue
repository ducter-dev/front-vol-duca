...
import FormAlmacen from '../components/FormAlmacen.vue'

const router = useRouter()
const { addToast } = useToast()
const { emit, bus } = useEventsBus()
const loader = ref(false)
const { insertAlmacen } = useAlmacenes()

/* Declaración de métodos */
async function onSubmit(form) {
  loader.value = true
  const { data, status, message } = await insertAlmacen(form)
  
  if (status == 201) {
    addToast({
      message: {
        title: "Éxito!",
        message: `Se agregó el almacén virtual de ${data.cliente.nombreCliente} a nuestros registros.`,
        type: "success"
      },
    })
    loader.value = false
