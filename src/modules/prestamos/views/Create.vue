...
import FormPrestamos from '../components/FormPrestamos.vue'

const router = useRouter()
const { addToast } = useToast()
const { emit, bus } = useEventsBus()
const loader = ref(false)
const { insertPrestamo } = usePrestamo()
...
async function onSubmit(form) {
  ...
  if (status == 201) {
    addToast({
      message: {
        title: "Éxito!",
        message: `Se agregó el préstamo de ${data.prestamo.cliente_venta.nombreCliente} a ${data.prestamo.cliente_compra.nombreCliente} a nuestros registros.`,
        type: "success"
      },
    })
    loader.value = false
    ...
    addToast({
      message: {
        ...
        message: message,
        type: "error",
        component: "create prestamo - onSubmit()"
      },
    })
  }
}
